...
|-------|---------|------|---------|---------|
| **Human** | 决策和审查 | - | - | - |
| **Claude** | #085 底部状态栏 | 🔄 进行中 | 2025-12-24 | - |
| **KIMI** | 无 | 💤 空闲 | - | - |
| **Kiro** | 无 | 💤 空闲 | - | - |
| **Kilo** | 无 | 💤 空闲 | - | - |
...
- [ ] **#085** 实现底部状态栏组件 `@Claude` `开始:2025-12-24` `预计:1h`
  - 描述：在 AppLayout 中添加底部状态栏，显示监控状态、活跃任务数、最近变化时间
  - 设计参考：`状态栏: 监控状态 ● 运行中 | 任务: 3/5 | 最近变化: 2分钟前`
  - 验收：状态栏固定在底部，实时显示监控状态

- [ ] **#086** 实现 Dashboard 监控控制按钮 `@无人认领` `预计:1h`
  - 描述：在 Dashboard 页面添加"开始监控/停止监控"按钮，调用 /api/monitor/start 和 /api/monitor/stop
  - 验收：点击按钮可启动/停止监控，按钮状态与监控状态同步

- [ ] **#087** 实现历史记录导出功能 `@无人认领` `预计:2h`
  - 描述：在 History 页面添加"导出"按钮，支持导出为 JSON/CSV 格式
  - 验收：可导出当前筛选结果，文件格式正确

- [ ] **#088** 实现 Settings 关于页面 `@无人认领` `预计:1h`
  - 描述：在 Settings 添加"关于"标签页，显示版本信息、项目链接、依赖信息
  - 验收：显示 WebMon 版本、GitHub 链接、许可证信息
