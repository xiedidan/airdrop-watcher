...
- [x] **#079** 实现配置管理 API `@Claude` `完成:2025-12-22` `实际:1.5h`
  - 描述：实现 /api/settings 接口，支持获取和更新各配置段
  - 验收：可读写通知、AI、监控等配置

- [x] **#080** 实现 Settings 页面 `@Claude` `完成:2025-12-22` `实际:2h`
  - 描述：实现系统设置页面，包含通知设置、AI 配置、监控参数等面板
  - 验收：所有配置可查看和修改，支持测试通知、测试 AI 连接
  - 注意：AI 配置面板需包含默认用户提示词编辑，并说明支持的占位符

- [x] **#081** 实现通知测试功能 `@Claude` `完成:2025-12-22` `实际:1h`
  - 描述：实现 /api/notification/test 接口，支持发送测试通知
  - 验收：可选择平台发送测试通知，返回发送结果

#### Phase 4: 部署与优化

- [x] **#082** 编写 Dockerfile `@Claude` `完成:2025-12-22` `实际:1h`
  - 描述：多阶段构建，第一阶段构建前端，第二阶段基于 Python 镜像
  - 验收：docker build 成功，镜像可运行
  - 实现内容：
    - 创建多阶段 Dockerfile
    - 阶段1: Node 20 Alpine 构建前端
    - 阶段2: Python 3.11 slim 运行后端
    - 安装 Playwright Chromium 浏览器
    - 支持非 root 用户运行
    - 添加健康检查
    - 创建 .dockerignore 优化构建
  - 注意：首次构建需要良好的网络环境下载依赖

- [x] **#083** 编写 docker-compose.yml `@Claude` `完成:2025-12-22` `实际:0.5h`
  - 描述：编排 webmon 服务，支持配置挂载、数据持久化、环境变量
  - 验收：docker-compose up -d 一键启动服务

- [x] **#084** 实现 CLI web 子命令 `@Claude` `完成:2025-12-22` `实际:0.5h`
